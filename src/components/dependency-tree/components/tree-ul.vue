<template>
  <li class="node-tree__item node-tree-wrap">
    <!-- <span>ul{{nodeData.isLeaf}}-{{ includes(expandKeysList, nodeData.nodeKey)}}{{expandKeysList}}</span> -->
    <div class="node-tree__title" @click.stop="handleClickNode(nodeData)">
      ...
      <!-- <span v-if="showDebug"
        >ul {{ includes(expandKeysList, nodeData.nodeKey) }} isLeaf:{{
          nodeData.isLeaf
        }}
        nodeKey: {{ nodeData.nodeKey }} deps: {{ nodeData.deps.length }}</span
      > -->
      <span
    ...
    <template v-if="!nodeData.isLeaf && isExpanded && nodeData.deps.length">
...
<script lang="ts" setup>
  import { includes, remove, cloneDeep, map, toLower } from 'lodash';
  import {
    PropType,
    inject,
    ref,
  ...
  import treeLI from './tree-li.vue';
  import useExpandNode from '../hooks/use-expand-node';
  ...
  const isMainLeafNode = ref(false);
  ...
  const emitter = defineEmits(['expand', 'update:expandKeys', 'loadmore']);
  ...
  const handleAddExpandKey = () => {
    const expandKeys = cloneDeep(expandKeysList.value);
    expandKeys.push(props.nodeData?.nodeKey);
    emitter('expand', true, [...expandKeys]);
  };
  ...
  const setRenderCondition = () => {
    isMainLeafNode.value =
      includes(expandKeysList.value, props.nodeData.nodeKey) &&
      !props.nodeData.isLeaf;
