<template>
  <div class="container policy-edit">
    <a-spin :loading="pageLoading" style="width: 100%" :size="50">
      <a-card class="general-card" :bordered="false">
        ...
        >
          ...
          <a-form-item
            ...
            field="name"
            :rules="[
              { required: true, message: $t('policy.detail.rules.name') },
            ]"
          >
          ...
          >
            <a-select v-model="formData.type" @change="handleTypeChange">
              <a-option
                ...
                :value="item.value"
                :label="
                  setLabel(`${formData.category}${item.value}`, item.value)
                "
              ></a-option>
            </a-select>
          </a-form-item>
          <a-form-item
            ...
            :rules="[
              {
                required: true,
                message: $t('policy.detail.rules.description'),
              },
            ]"
          >
            <a-textarea
          ...
          <a-form-item
            :label="$t('policy.table.switch')"
            class="no-bottom-gap"
            :disabled="false"
          >
            <a-switch v-model="formData.enabled" size="small" />
          </a-form-item>
          ...
          <a-space v-if="useInScope === 'partial'" :size="0">
            ...
            >
              ...
              <a-space v-if="formData.category === 'sca'" direction="vertical">
                <span class="s-label"
                  >{{ $t('policy.detail.edit.proxy') }}:</span
                >
                <a-select
            ...
            >
              <div class="conditions-box">
                ...
                >
                    ...
                    >
                      ...
                      >
                        ...
                        >
                          <a-tooltip
                            :content="
                              setLabel(sItem.label, sItem.conditionType)
                            "
                          >
                            <span>{{
                              setLabel(sItem.subLabel, sItem.conditionType)
                            }}</span>
                          </a-tooltip>
                        </a-option>
                    ...
                    >
                      ...
                      >
                        <a-select
                          ...
                          @change="
                            (val) => handleItemOperatorChange(val, cItem)
                          "
                        >
                          <a-option
                      ...
                      >
                        ...
                        >
                          <a-select
                            ...
                            allow-clear
                            :multiple="
                              multipleOperator.includes(cItem.operator)
                            "
                          >
                            <a-option
                              v-for="(
                                vItem, vIndex
                              ) in getConditionValueOptions(cItem)"
                              :key="vIndex"
                              :value="vItem.value"
                  ...
                  <a-divider
                    v-show="
                      cIndex !== formData.constraint.conditions.length - 1
                    "
                    style="margin-top: 0"
                  ></a-divider>
                </div>
              ...
              <div class="container-wrap" :class="{ expr: format === 'expr' }">
                ...
                >
                  ...
                  <aceEditor
                    ...
                    :source="completerData"
                    lang="javascript"
                    :read-only="!!expressionEditable"
                    @click="handleClickDisable"
                    @change="handleExpressionInput"
                  ></aceEditor>
                  <template #extra>
                    <div>
                      <div
                        >{{ $t('policy.detail.edit.exprRef') }}：<a-link
                          href="https://seal-io.github.io/docs/"
                          target="_blank"
                          >https://seal-io.github.io/docs/</a-link
                        ></div
                      >
...
<script lang="ts" setup>
  ...
  type localeType = 'zh-CN' | 'en-US' | undefined;
  ...
  const categoriesList = computed(() => {
    ...
    const list = policyTypeCategory.value[key] || [];
  ...
  const policyCategoryConditionList = computed(() => {
    const categoryData = policyTypeList.value.find((item) => {
  ...
  const setLabel = (text, value?) => {
    const ln = locale.value as localeType;
    const localsMap = {
      ...typeMap,
      ...customTypeMap.value,
    };
    const localeText = get(localsMap, `${lowerFirst(text)}`);
    console.log('localeText===', localsMap, localeText, text);
    if (localeKey(localeText)) return t(localeText);
    return value ?? text;
  };
  // computed=====end
    ...
    conditionList.value = map(keys(policyCategoryData), (key) => {
      ...
      const arr: Array<ConditionItemConfig> = map(
        ...
        (conditionKey) => {
          ...
          console.log(
            'number===',
            get(conditionData, 'properties.value.type'),
            conditionData
          );
          return {
  ...
  const getLicenseGroups = async () => {
    const { data } = await commonRequestList('licenses');
    const list = get(data, 'items') || [];
  ...
  onMounted(async () => {
    try {
      ...
      const list = await getLicenseGroups();
      await getPluginPolicySchemas(list);
      await fetchData();
